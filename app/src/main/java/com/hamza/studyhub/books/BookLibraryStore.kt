...
class BookLibraryStore(private val context: Context) : BookCatalog {
    private val file: File
        get() = File(context.filesDir, "book_library.json")
    ...
    override fun allBooks(): List<BookAsset> {
        ...
        return buildList {
            for (i in 0 until items.length()) {
                items.optJSONObject(i)?.toBookAsset()?.let(::add)
            }
    ...
    fun delete(bookId: String) {
        write(allBooks().filterNot { it.id == bookId })
    }
    ...
    private fun BookAsset.toJson(): JSONObject = JSONObject().apply {
        ...
        put("title", title)
        put("subject", subject)
        put("edition", edition.orEmpty())
        put("localUri", localUri.orEmpty())
        put("mimeType", mimeType.orEmpty())
        put("pageCount", pageCount ?: 0)
    ...
    private fun JSONObject.toBookAsset(): BookAsset? {
        ...
        val aliases = buildSet {
            for (i in 0 until aliasesJson.length()) {
                aliasesJson.optString(i).takeIf { it.isNotBlank() }?.let(::add)
            }
        }
        return BookAsset(
            id = id,
