...
class SourceHubActivity : AppCompatActivity() {
    ...
    private fun buildScreen(): View {
        val content = LinearLayout(this).apply {
            ...
            setPadding(HamzaUi.dp(this@SourceHubActivity, 18), HamzaUi.dp(this@SourceHubActivity, 20), HamzaUi.dp(this@SourceHubActivity, 18), HamzaUi.dp(this@SourceHubActivity, 30))
            setBackgroundColor(HamzaUi.bg)
        }
        content.addView(HamzaUi.title(this, "المصادر والمزامنة"))
        content.addView(HamzaUi.subtitle(this, "راقب صحة WebUntis وTeams والتنبيهات من مكان واحد."), HamzaUi.marginTop(this, 4))
        ...
        metrics.addView(newCountText, weight(4)); metrics.addView(attentionCountText, weight(4)); metrics.addView(sourcesCountText, weight(0))
        ...
        content.addView(sourceCard("WebUntis", "المصدر الرسمي للواجبات والمواعيد المسجلة في Untis", HamzaUi.green) {
            untisStatus = it
            HamzaUi.secondaryButton(this, "مزامنة Untis الآن", HamzaUi.green).apply {
                ...
                setOnClickListener {
                    if (WebUntisConfigStore.isConfigured(this@SourceHubActivity)) {
                        WebUntisSyncWorker.syncNow(this@SourceHubActivity); untisStatus.text = "جاري طلب مزامنة WebUntis…"
                    } else startActivity(Intent(this@SourceHubActivity, LaunchActivity::class.java))
                }
            }
        })
        content.addView(sourceCard("Microsoft Teams", "Notification للسرعة + Deep Sync لالتقاط الواجبات حتى لو لم يصل إشعار", HamzaUi.purple) {
            teamsStatus = it
            HamzaUi.secondaryButton(this, "إدارة Teams Deep Sync", HamzaUi.purple).apply {
                tag = "source_teams_manage"; setOnClickListener { startActivity(Intent(this@SourceHubActivity, TeamsConnectionActivity::class.java)) }
            }
        })

        content.addView(HamzaUi.primaryButton(this, "فتح الواجبات").apply {
            tag = "source_open_feed"; setOnClickListener { startActivity(Intent(this@SourceHubActivity, MainActivity::class.java)) }
    ...
    private fun notificationCard(): View {
        ...
        notificationAction = HamzaUi.primaryButton(this, "تفعيل التنبيهات").apply {
            ...
            setOnClickListener {
                ...
                else refreshNotificationHealth()
            }
        }
        notificationTest = HamzaUi.secondaryButton(this, "اختبار إشعار", HamzaUi.green).apply {
            tag = "notifications_test"
            setOnClickListener { BackgroundAlert.notify(this@SourceHubActivity, "Hamza Study Hub", "التنبيهات تعمل بنجاح.", 4199) }
        }
        row.addView(notificationAction, LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f).apply { marginEnd = HamzaUi.dp(this@SourceHubActivity, 5) })
        row.addView(notificationTest, LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f).apply { marginStart = HamzaUi.dp(this@SourceHubActivity, 5) })
        ...
        body.addView(HamzaUi.secondaryButton(this, "إعدادات إشعارات التطبيق").apply {
            ...
            setOnClickListener { startActivity(Intent(Settings.ACTION_APP_NOTIFICATION_SETTINGS).apply { putExtra(Settings.EXTRA_APP_PACKAGE, packageName) }) }
    ...
    private fun sourceCard(title: String, subtitle: String, accent: Int, builder: (TextView) -> View): View {
        ...
        card.layoutParams = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT).apply { bottomMargin = HamzaUi.dp(this@SourceHubActivity, 10) }
        return card
    }

    private fun refresh() {
        refreshMetrics(); refreshNotificationHealth()
        if (::untisStatus.isInitialized) {
            val last = WebUntisConfigStore.lastSync(this); val error = WebUntisConfigStore.lastError(this)
        ...
        if (::teamsStatus.isInitialized) {
            val account = TeamsAuthStore.account(this); val last = TeamsAuthStore.lastSync(this); val error = TeamsAuthStore.lastError(this)
    ...
    private fun refreshMetrics() {
        ...
        val sourceCount = listOf(WebUntisConfigStore.isConfigured(this), TeamsAuthStore.account(this) != null).count { it }
        if (::newCountText.isInitialized) newCountText.text = "$newCount\nالجديد"
        if (::attentionCountText.isInitialized) attentionCountText.text = "$attention\nيحتاج انتباه"
        if (::sourcesCountText.isInitialized) sourcesCountText.text = "$sourceCount/2\nمصادر"
        if (::focusText.isInitialized) {
            val today = LocalDate.now(); val tomorrow = today.plusDays(1)
            val overdue = items.count { !it.optBoolean("completed", false) && parseDueDate(it.optInt("dueDate", 0))?.isBefore(today) == true }
            val urgent = items.count {
                val due = parseDueDate(it.optInt("dueDate", 0))
                !it.optBoolean("completed", false) && due != null && !due.isBefore(today) && !due.isAfter(tomorrow)
            }
            focusText.text = when {
    ...
    private fun refreshNotificationHealth() {
        ...
        val granted = Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU || ContextCompat.checkSelfPermission(this, Manifest.permission.POST_NOTIFICATIONS) == PackageManager.PERMISSION_GRANTED
        notificationStatus.text = if (granted) "التنبيهات مفعّلة للتطبيق." else "التنبيهات غير مفعّلة للتطبيق نفسه."
    ...
    private fun parseDueDate(value: Int): LocalDate? = if (value <= 0) null else runCatching { LocalDate.parse(value.toString(), DateTimeFormatter.BASIC_ISO_DATE) }.getOrNull()
    private fun metric(label: String, bg: Int, fg: Int) = HamzaUi.softPill(this, "0\n$label", bg, fg).apply { minHeight = HamzaUi.dp(this@SourceHubActivity, 64); gravity = Gravity.CENTER }
    private fun weight(end: Int) = LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f).apply { marginEnd = HamzaUi.dp(this@SourceHubActivity, end) }
    private fun formatTime(timestamp: Long) = SimpleDateFormat("dd/MM • hh:mm a", Locale.getDefault()).format(Date(timestamp))
}
