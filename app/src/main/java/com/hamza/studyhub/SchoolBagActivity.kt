...
class SchoolBagActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val root = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; gravity = Gravity.END; setPadding(24,24,24,24); setBackgroundColor(HamzaUi.bg) }
        ...
        val status = TextView(this).apply { text = "جاري قراءة جدول Untis…"; gravity = Gravity.END; textSize = 16f; setPadding(8,24,8,24) }
        root.addView(status)
        setContentView(root)
                ...
                result.onSuccess { lessons ->
                    ...
                    if (active.isEmpty()) {
                        ...
                        active.distinctBy { it.subject.ifBlank { "lesson-${it.id}" } }.forEach { lesson ->
                            ...
                            root.addView(HamzaUi.card(this, title, detail))
                        }
                        root.addView(HamzaUi.statusBox(this, "ضع في الشنطة مواد الحصص الظاهرة فقط. الحصص الملغاة مستبعدة تلقائيًا."))
                    }
