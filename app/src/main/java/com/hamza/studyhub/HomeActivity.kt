...
class HomeActivity : AppCompatActivity() {
    ...
    private fun actionCard(title:String,subtitle:String,buttonText:String,action:()->Unit):View{val card=HamzaUi.card(this);val body=HamzaUi.cardContent(card);body.addView(HamzaUi.title(this,title,18f));body.addView(HamzaUi.subtitle(this,subtitle),HamzaUi.marginTop(this,4));body.addView(HamzaUi.secondaryButton(this,buttonText).apply{tag="home_${title.hashCode()}";setOnClickListener{action()}},HamzaUi.marginTop(this,10));card.layoutParams=LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,LinearLayout.LayoutParams.WRAP_CONTENT).apply{bottomMargin=HamzaUi.dp(this@HomeActivity,10)};return card}
    private fun showLearningSummary(){val attempts=LearningEvidenceStore(this).attempts();val subjects=attempts.groupingBy{it.subject.ifBlank{"غير محدد"}}.eachCount();val body=if(attempts.isEmpty())"لسه مفيش محاولات مصوّرة. افتح واجب فيه صفحة وصوّر حل حمزة بعد ما يخلص." else buildString{append("عندنا ${attempts.size} محاولة محفوظة.\n\n");subjects.entries.sortedByDescending{it.value}.forEach{append("${it.key}: ${it.value}\n")};append("\nالتحليل المتقدم للأخطاء المتكررة هيتبني على الأدلة دي بدون تصحيح تلقائي لحمزة.")};AlertDialog.Builder(this).setTitle("ملف تعلّم حمزة").setMessage(body).setPositiveButton("تمام",null).show()}
    private fun readFeed():List<JSONObject>{val file=File(filesDir,"school_notifications.jsonl");if(!file.exists())return emptyList();return file.readLines().mapNotNull{runCatching{JSONObject(it)}.getOrNull()}}
    private fun formatTime(timestamp:Long)=SimpleDateFormat("dd/MM HH:mm",Locale.getDefault()).format(Date(timestamp))
}
