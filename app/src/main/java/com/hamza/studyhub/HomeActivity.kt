...
class HomeActivity : AppCompatActivity() {
    private lateinit var planSubtitle: TextView
    ...
    private fun buildScreen(): View {
        ...
        heroBody.addView(TextView(this).apply {
            ...
            gravity = Gravity.END
            setPadding(0, HamzaUi.dp(this@HomeActivity, 8), 0, 0)
        })
        heroBody.addView(TextView(this).apply {
            text = "Bereit für heute?  يلا نخلّص يومنا خطوة خطوة ⭐"
            textSize = 15f
        ...
        val plan = HamzaUi.card(this, radius = 24, padding = 18).apply {
            setCardBackgroundColor(0xFFFFF3D8.toInt())
            strokeColor = 0xFFFFD36A.toInt()
        }
        val planBody = HamzaUi.cardContent(plan)
        planBody.addView(TextView(this).apply {
            text = "📚  Mein Plan"
            textSize = 24f
            setTypeface(typeface, Typeface.BOLD)
        ...
        planSubtitle = TextView(this).apply {
            text = "خطة حمزة للمذاكرة • راحة + مذاكرة + تجهيز بكرة"
            textSize = 15f
            setTextColor(0xFF5D6572.toInt())
            gravity = Gravity.END
            setPadding(0, HamzaUi.dp(this@HomeActivity, 5), 0, HamzaUi.dp(this@HomeActivity, 10))
        }
        planBody.addView(planSubtitle)
        planBody.addView(HamzaUi.primaryButton(this, "▶  ابدأ خطة حمزة").apply {
            setOnClickListener { startActivity(Intent(this@HomeActivity, StudyPlanActivity::class.java)) }
        })
        ...
        content.addView(TextView(this).apply {
            text = "🇩🇪  Kleine Schritte. Jeden Tag.  •  خطوة صغيرة كل يوم"
            textSize = 13.5f
            gravity = Gravity.CENTER
            setTextColor(0xFF737B87.toInt())
        }, HamzaUi.marginTop(this, 20))
    ...
    private fun shortcut(icon: String, german: String, arabic: String, bg: Int, action: () -> Unit): View {
        val card = HamzaUi.card(this, radius = 22, padding = 15).apply {
    ...
    private fun tileParams(endMargin: Int) = LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f).apply {
