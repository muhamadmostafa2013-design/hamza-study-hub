...
/**
 * Append-only local evidence log for Hamza's own attempts.
 *
 * This is not a grade book. We preserve each attempt so later analysis can answer:
 * what did Hamza try, what pattern repeated, and did it improve in later work?
 */
class LearningEvidenceStore(private val context: Context) {
    private val attemptsFile: File
        get() = File(context.filesDir, "student_attempts.jsonl")
    ...
    fun attempts(): List<StudentAttempt> {
        ...
        return attemptsFile.readLines().mapNotNull { line ->
            runCatching { JSONObject(line).toAttempt() }.getOrNull()
        }
    }
    ...
    private fun StudentAttempt.toJson(): JSONObject = JSONObject().apply {
        put("id", id)
        put("assignmentFingerprint", assignmentFingerprint)
        put("subject", subject)
        put("bookId", bookId.orEmpty())
        put("page", page ?: 0)
        put("exercise", exercise.orEmpty())
        put("capturedAt", capturedAt)
        put("imageUri", imageUri.orEmpty())
        put("extractedText", extractedText.orEmpty())
